...
import { CheckCircle, MapPin, Calendar, Users, Mail, Shield, Flame } from "lucide-react"
...
export default function BookingSuccessPage() {
  ...
  const fetchBookingDetails = async () => {
    try {
      const response = await fetch(`/api/barbecue/booking-success?session_id=${sessionId}`)
      if (response.ok) {
  ...
  // Determine booking type based on available fields
  const isBBQBooking = booking && booking.groupSize !== undefined && !booking.numberOfTents
  const isCampingBooking = booking && booking.numberOfTents !== undefined
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="text-center mb-6">
          <div className="w-full flex justify-center">
            <div className="w-64">
              <DotLottieReact src="/bbq.lottie" loop autoplay />
            </div>
          </div>
            ...
            <CardContent className="p-4 space-y-3">
              ...
              {isBBQBooking ? (
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="font-medium text-[#3C2317] text-sm">Group Size</p>
                    <p className="text-[#3C2317]/80 text-sm">
            ...
            __html: `
              ...
              if (typeof gtag !== 'undefined') {
                gtag('event', 'purchase', {
                  ...
                  currency: 'AED',
                  items: [{
                    item_id: '${isBBQBooking ? "bbq" : "camping"}-${booking.location?.toLowerCase() || "desert"}',
                    item_name: '${isBBQBooking ? "Desert BBQ" : "Desert Camping"}${booking.location ? " - " + booking.location : ""}',
                    category: '${isBBQBooking ? "BBQ" : "Camping"}',
                    quantity: ${isBBQBooking ? booking.groupSize : booking.numberOfTents},
                    price: ${booking.total}
                  }]
                });
              }
              ...
              if (typeof fbq !== 'undefined') {
                fbq('track', 'Purchase', {
                  value: ${booking.total},
                  currency: 'AED',
                  content_ids: ['${isBBQBooking ? "bbq" : "camping"}-${booking.location?.toLowerCase() || "desert"}'],
